...
if [ "$ROLE" = "2" ]; then
    ...
    # --- persistent client identity (key + name) ---------------------------
    # A stable key so the host recognises this client across sessions and can
    # restore its assets. Generated once, stored in ~/.config/x4mp/client.key.
    # The host ALSO keeps a server-side backup of every client's key (recovery
    # if this file is lost). The name must be a single token (no spaces) because
    # the JOIN handshake is space-delimited; spaces become underscores.
    KEY_DIR="${HOME}/.config/x4mp"
    KEY_FILE="${KEY_DIR}/client.key"
    if [ ! -f "${KEY_FILE}" ]; then
        mkdir -p "${KEY_DIR}"
        NEW_KEY="$(cat /proc/sys/kernel/random/uuid 2>/dev/null || head -c16 /dev/urandom | od -An -tx1 | tr -d ' \n')"
        DEF_NAME="${USER:-$(hostname)}"
        read -r -p "Player name for this client [${DEF_NAME}]: " CLIENT_NAME
        CLIENT_NAME="${CLIENT_NAME:-${DEF_NAME}}"
        CLIENT_NAME="${CLIENT_NAME// /_}"
        { echo "key=${NEW_KEY}"; echo "name=${CLIENT_NAME}"; } > "${KEY_FILE}"
        echo ">> Created client identity: ${KEY_FILE} (key ${NEW_KEY:0:8}...)"
    fi
    export X4MP_CLIENT_KEY="$(grep -E '^key=' "${KEY_FILE}" | head -1 | cut -d= -f2-)"
    export X4MP_CLIENT_NAME="$(grep -E '^name=' "${KEY_FILE}" | head -1 | cut -d= -f2-)"
    echo ">> Client identity: name=${X4MP_CLIENT_NAME} key=${X4MP_CLIENT_KEY:0:8}..."
